#!/usr/bin/env node
// MemWeave Codex plugin -- UserPromptSubmit hook (cross-platform Node).
//
// Reads a CC-style event from stdin (Codex's UserPromptSubmit is a
...
// token-budgeted XML chunk. We hand it to Codex as
// `hookSpecificOutput.additionalContext` so the LLM sees it before
// responding.
//
// We deliberately do NOT track `alreadyInjected` here -- the Codex
...
// 4. Emit the hook output. Codex reads the LAST line of stdout as
