// MemWeave Mavis plugin -- shared library for hook scripts.
//
...
// fail-silent -- a MemWeave outage must never break the Mavis agent.
...
/**
 * Derive a session id from the Mavis hook event. Mavis mirrors Claude
 * Code's CC marketplace format, so we read the snake_case fields first
 * and fall back to camelCase variants + a stable hash of cwd so a
 * missing session_id never produces an empty string.
 */
export function deriveSessionId(event) {
  ...
  if (event.sessionID) return String(event.sessionID);
  // Stable fallback: hash of cwd. Same project = same id, retries
  // collapse to the same row server-side.
  const cwd = event.cwd || process.cwd();
  return `mavis-${createHash('sha256').update(cwd).digest('hex').slice(0, 16)}`;
}
...
export function makeMessageId(sessionId, role, content) {
  ...
  return `mavis-${sessionId}-${role}-${hash}`;
}

/**
 * Extract file paths from a CC-style tool_input. Mavis uses the same
 * keys as the opencode plugin: filePath, file_path, path, file, pattern.
 */
...
/**
 * Write a CC-marketplace hook output JSON line to stdout. Mavis reads
 * the LAST line of stdout as the hook output, so this is safe to call
 * for any hook.
 */
