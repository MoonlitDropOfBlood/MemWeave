...
export function MemoryDetailPage() {
  ...
  if (memoryQ.isLoading) return <div className={styles.loading}><span className="spinner" /> Loading…</div>;
  if (memoryQ.error) return <div className={styles.error}>Not found</div>;
    ...
    <article className={styles.page}>
      ...
      <section className={styles.section}>
        ...
        <dl className={styles.dl}>
          <dt>Strength</dt><dd><StrengthBar value={m.strength} /></dd>
          <dt>importance</dt><dd>{m.importance}</dd>
          <dt>confidence</dt><dd>{m.confidence.toFixed(2)}</dd>
          <dt>Access count</dt><dd>{m.accessCount}</dd>
          <dt>Source</dt><dd>{m.source}</dd>
          <dt>Created</dt><dd>{formatDate(m.createdAt)}</dd>
          {m.lastAccessedAt && <><dt>Last accessed</dt><dd>{formatDate(m.lastAccessedAt)}</dd></>}
        </dl>
      </section>
      ...
      <section className={styles.section}>
        <div className={styles.tabs}>
          ...
          >Graph ({graphQ.data?.nodes.length ?? 0})</button>
          <button
            className={tab === 'access' ? `${styles.tab} ${styles.tabActive}` : styles.tab}
            onClick={() => setTab('access')}
          >Access logs ({logsQ.data?.logs.length ?? 0})</button>
        </div>

        {tab === 'graph' ? (
          <div className={styles.graphPanel}>
            {(graphQ.data?.edges.length ?? 0) === 0 ? (
              <p className={styles.empty}>No connected edges.</p>
            ) : (
              <ul className={styles.edgeList}>
                {graphQ.data!.edges.map((e) => (
                  <li key={e.id} className={styles.edgeItem}>
                    ...
                    <span className={styles.edgeStrength}>strength {e.strength.toFixed(2)}</span>
          ...
          <div className={styles.logsPanel}>
            {(logsQ.data?.logs.length ?? 0) === 0 ? (
              <p className={styles.empty}>No access recorded yet.</p>
            ) : (
              <table className={styles.logTable}>
                <thead>
                  <tr>
                    <th>When</th><th>Source</th><th>Used in context</th><th>Query</th>
                  </tr>
                </thead>
                <tbody>
