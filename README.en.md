...
- **MCP tool set**: 10 `memory_*` tools for the agent to pull/write/search memories, closing the progressive-disclosure loop.
...
The plugin auto-registers MCP tools while injecting summaries. In OpenCode, the LLM can call `memory_expand` immediately — no extra config. Other clients configure `npx @mem-weave/mcp` manually for the same effect.
