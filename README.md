...
MemWeave 以三个 npm 包发布：

| 包 | 用途 | 安装后产生 |
|---|---|---|
| [`@mem-weave/server`](https://www.npmjs.com/package/@mem-weave/server) | Fastify + SQLite 主体服务 + CLI | `memweave` 全局命令 |
| [`@mem-weave/mcp`](https://www.npmjs.com/package/@mem-weave/mcp) | stdio MCP 服务器，10 个 memory_* 工具 | `memweave-mcp` 全局命令 |
| [`@mem-weave/opencode-plugin`](https://www.npmjs.com/package/@mem-weave/opencode-plugin) | OpenCode 插件，自动注入 + 自动注册 MCP | OpenCode 加载时生效 |
...
然后在 `~/.config/opencode/opencode.json` 的 `plugin` 数组里加 `"@mem-weave/opencode-plugin"`。
...
通过 npm 全局安装后可直接使用 `memweave` 命令（源码开发则用 `npm run cli -- <command>`）。
...
`@mem-weave/opencode-plugin` 提供了一个 OpenCode 插件 `MemweaveInjectPlugin`，能在 Agent 与 LLM 交互时**自动注入相关记忆**，无需 LLM 主动调用工具。
...
1. 插件加载 → `config` 钩子跑，向 OpenCode 注册 `mcp["memweave"]` = `npx -y @mem-weave/mcp`
